...
namespace tiki
{
	...
	static const uint8 s_aVirtualKeyCodeMapping[] =
	{
	...
	TIKI_COMPILETIME_ASSERT( TIKI_COUNT( s_aVirtualKeyCodeMapping ) == KeyboardKey_Count );

	static const uint16 s_aControllerButtonMapping[] =
	{
	...
	TIKI_COMPILETIME_ASSERT( TIKI_COUNT( s_aControllerButtonMapping ) == ControllerButton_Count );
	...
	bool input::initializeKeyboard( InputSystemPlatformData& platformData, const InputSystemParameters& parameters )
	{
		...
		if( SUCCEEDED( result ) && platformData.pKeyboard != nullptr )
		{
			...
			{
				result = platformData.pKeyboard->SetCooperativeLevel( (HWND)parameters.windowHandle, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE );
				if( SUCCEEDED( result ) )
				{
					return true;
				}
	...
	bool input::initializeMouse( InputSystemPlatformData& platformData, const InputSystemParameters& parameters )
	{
		...
		{
			result = platformData.pMouse->SetDataFormat( &c_dfDIMouse2 );
			if( SUCCEEDED( result ) )
			{
