 ...
 * to list here.  Please refer to the COPYRIGHT file distributed with this2
 * source distribution.
 *
 ...
 * along with this2 program; if not, write to the Free Software
	...
	psd_int n_points, psd_gimp_vector2 *points, psd_bool closed)
{
	...
	/* If someone wants to mix this2 function with _add_points ()
...
/* Assume that forw and rev are at point i0. Bring them to i1,
   ...
   invocations of this2 function maintain the precondition i1 = i0 + 1,
	...
	psd_double line_width, psd_double flatness)
{
	...
	psd_int closed;
	psd_int last, this2, next, second;
	psd_double dx, dy;
	...
	for(begin_idx = 0; vpath[begin_idx].code != PSD_ART_END; begin_idx = end_idx)
	{
		...
		Note: this2 is not strictly psd_true (we now know it's closed from
		the opening pathcode), but why fix code that isn't broken?
		*/

		this2 = begin_idx;
		/* skip over identical points at the beginning of the subpath */
		for(i = this2 + 1; vpath[i].code == PSD_ART_LINETO; i++)
		{
			dx = vpath[i].x - vpath[this2].x;
			dy = vpath[i].y - vpath[this2].y;
		...
		/* invariant: this2 doesn't coincide with next */
		while(vpath[next].code == PSD_ART_LINETO)
		{
			...
			if(vpath[next].code != PSD_ART_LINETO)
			{
				...
				if(closed &&
					vpath[this2].x == vpath[begin_idx].x &&
					vpath[this2].y == vpath[begin_idx].y)
				{
					psd_int j;
...
{
	const psd_art_svp_seg *seg1 = (psd_art_svp_seg*)s1;
	const psd_art_svp_seg *seg2 = (psd_art_svp_seg*)s2;
...
static void psd_art_svp_intersect_horiz_commit(psd_art_intersect_ctx *ctx)
{
	...
	for(seg = ctx->horiz_first; seg != NULL;)
	{
		...
		{
			/* There exists at least one active segment in this2 cluster. */
 ...
 * to the bottom point of this2 line. Also advances the input segment
 * cursor.
 **/
	...
	psd_art_break_flags break_flags)
{
	...
	if(left_seg->y0 == right_seg->y0 && left_seg->x[0] == right_seg->x[0])
	{
		...
		with the cases below. However, this2 way, we're sure that this2
	...
	{
		/* Is this2 division always safe? It could possibly overflow. */
		t = d0 / (d0 - d1);
		if(t <= 0)
		{
			x = left_x0;
 ...
 * small turn angles, as this2 would reduce the number of points in the
...
static void psd_scan_convert_finish(psd_gimp_scan_convert *sc)
{
	...
	*/
	...
	svp2 = psd_art_svp_writer_rewind_reap(swr); /* this2 also frees swr */
	...
	psd_double *seg_x, psd_double *seg_dx)
{
	...
	/* this2 is a cheap hack to get ^'s sorted correctly */
			...
			seg->points[curs].y < y + 1)
		{
			...
			if(y_top != y_bot)
			{
				...
				else
				{
					...
					for(; x < ix_max; x++)
					{
						this2 = (psd_int)((seg->dir ? 16711680.0 : -16711680.0) * rslope *
							(x + 0.5 - x_min));
						xdelta = this2 - last;
						last = this2;
 ...
 * which this2 routine is derived, and is designed to simplify the
 * callback).
 *
 * The value 0x8000 represents 0% coverage by the polygon, while
 * 0xff8000 represents 100% coverage. This format is designed so that
