...
namespace tiki
{
	void AnimationSplineKey::sampleHermite( float* pTarget, uint targetCapacity, const AnimationSplineKey& leftKey, const AnimationSplineKey& rightKey, float t )
	{
	...
	void AnimationSpline::createFromKeys( const AnimationSplineKey* pSourceKeys, uint keyCount, uint dimensions, uint lengthInFrames )
	{
		...
		m_keys = List< AnimationSplineKey >( pSourceKeys, keyCount );
	}

	void AnimationSpline::createFromSamples( const float* pSource, uint dimensions, uint lengthInFrames, float threshold )
	{
		TIKI_ASSERT( dimensions <= SplineLimits_MaxDimensions );
		...
		AnimationSplineKey firstKey = createKey( pSource, 0u, dimensions );
		m_keys.add( firstKey );
		...
		AnimationSplineKey lastKey = createKey( pSource, lengthInFrames - 1u, dimensions );
		m_keys.add( lastKey );
		...
		do
		{
			...
			{
				const AnimationSplineKey& leftKey = m_keys[ i - 1u ];
				const AnimationSplineKey& rightKey = m_keys[ i ];
				...
				for (uint time = leftKey.time; time < rightKey.time; ++time)
				{
					...
					AnimationSplineKey::sampleHermite( currentValue, dimensions, leftKey, rightKey, koeff );
	...
	void AnimationSpline::dispose()
	{
		m_keys.dispose();
	}

	void AnimationSpline::sample( float* pTarget, uint targetCapacity, float sampleTime ) const
	{
		...
		const AnimationSplineKey* pLeftKey	= &m_keys[ 0u ];
		const AnimationSplineKey* pRightKey	= &m_keys[ 1u ];
		...
		AnimationSplineKey::sampleHermite( pTarget, targetCapacity, *pLeftKey, *pRightKey, time );
	}

	void AnimationSpline::insertKey( const AnimationSplineKey& key )
	{
		for (uint i = 0; i < m_keys.getCount(); ++i)
		{
			const AnimationSplineKey& currentKey = m_keys[ i ];
	...
	AnimationSplineKey AnimationSpline::createKey( const float* pSource, uint time, uint dimensions )
	{
		const float* pSourceKey = pSource + ( time * dimensions );

		AnimationSplineKey key;
	...
	void AnimationSpline::calculateTangents( const float* pSource, uint sourceLength, uint dimensions )
	{
		const float t = 0.5f;

		for (uint i = 0; i < m_keys.getCount(); ++i)
		{
			AnimationSplineKey& key1			= m_keys[ i ];
			const AnimationSplineKey& key2	= m_keys[ i == m_keys.getCount() - 1u ? i : i + 1u ];
