...
namespace tiki
{
	...
	void Url::setAuthData( const string& username, const string& password )
	{
		...
		const string authString = formatDynamicString( "%s:%s@", username.cStr(), password.cStr() );
	...
	void Url::setPort( const string& port )
	{
		...
		const string portString = formatDynamicString( ":%s", port.cStr() );
	...
	void Url::setQueryValue( const string& key, const string& value )
	{
		...
		{
			const string oldPair = formatDynamicString( "%s=%s", key.cStr(), oldValue.cStr() );
			const string newPair = formatDynamicString( "%s=%s", key.cStr(), value.cStr() );
			m_urlString = m_urlString.replace( oldPair, newPair );
		}
		else
		{
			const int fragmentIndex = (int)m_urlString.getLength() - m_fragmentLength - 1;
			const string newPair = formatDynamicString( "%s%s=%s", ( m_query.isEmpty() ? "?" : "&" ), key.cStr(), value.cStr() );
	...
	void Url::setFragment( const string& fragment )
	{
		...
		m_urlString += formatDynamicString( "#%s", fragment.cStr() );
