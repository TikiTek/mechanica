...
namespace tiki
{
	...
	Vector2 DebugGuiAlignment::getMinimumSize()
	{
		...
		{
			const float modAlign = f32::mod( getRectangle().x, m_alignment.x );
			minSize.x = m_alignment.x - modAlign;
		}

		if ( isBitSet(m_flags, OrientationFlags_Y ) )
		{
			const float modAlign = f32::mod( getRectangle().y, m_alignment.y );
	...
	void DebugGuiAlignment::handleRectangleChanged( const Rectangle& /*boundingRectangle*/ )
	{
	}
}
