 ...
 */
static void smpt_state(struct connectdata *conn, smtpstate newstate)
{
  struct smtp_conn *smtpc = &conn->proto.smtpc;
...
static CURLcode smtp_perform_upgrade_tls(struct connectdata *conn)
{
  ...
  if(!result) {
    if(smtpc->state != SMTP_UPGRADETLS)
      smpt_state(conn, SMTP_UPGRADETLS);
                                     ...
                                     smtpstate instate)
{
  ...
  if(!result)
    switch(progress) {
    case SASL_DONE:
      smpt_state(conn, SMTP_STOP);  /* Authenticated */
      break;
...
static CURLcode smtp_connect(struct connectdata *conn, bool *done)
{
  ...
  *done = FALSE; /* default to not done yet */
  ...
  smpt_state(conn, SMTP_SERVERGREET);
