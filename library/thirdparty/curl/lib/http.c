...
CURLcode Curl_http_auth_act(struct connectdata *conn)
{
  ...
  if(pickhost || pickproxy) {
    ...
    if((data->set.httpreq != CURL_HTTPREQ_GET) &&
       (data->set.httpreq != CURL_HTTPREQ_HEAD) &&
                        ...
                        CURLcode status, bool premature)
{
  ...
  if(CURL_HTTPREQ_POST_FORM == data->set.httpreq) {
  ...
  else if(CURL_HTTPREQ_PUT == data->set.httpreq)
    data->req.bytecount = http->readbytecount + http->writebytecount;
      ...
      if(ptr) {
        ...
        if(*ptr) {
          ...
          if(conn->allocptr.host &&
            ...
            ;
          else if(data->set.httpreq == CURL_HTTPREQ_POST_FORM &&
...
CURLcode Curl_http(struct connectdata *conn, bool *done)
{
  ...
  *done = TRUE;
  ...
  if((data->state.authhost.multi || data->state.authproxy.multi) &&
     (httpreq != CURL_HTTPREQ_GET) &&
     (httpreq != CURL_HTTPREQ_HEAD)) {
...
#define TE_HEADER "TE: gzip\r\n"
  ...
  if(CURL_HTTPREQ_POST_FORM == httpreq) {
  ...
  if(( (CURL_HTTPREQ_POST == httpreq) ||
       (CURL_HTTPREQ_POST_FORM == httpreq) ||
       (CURL_HTTPREQ_PUT == httpreq) ) &&
  ...
  if(data->state.use_range) {
     ...
     */
    if(((httpreq == CURL_HTTPREQ_GET) || (httpreq == CURL_HTTPREQ_HEAD)) &&
    ...
    else if((httpreq != CURL_HTTPREQ_GET) &&
  ...
  case CURL_HTTPREQ_PUT: /* Let's PUT the data to the server! */
      ...
      if(nc) {
        ...
        if(data->set.http_fail_on_error && (k->httpcode >= 400) &&
          ...
          if(data->state.resume_from &&
             (data->set.httpreq==CURL_HTTPREQ_GET) &&
             (k->httpcode == 416)) {
