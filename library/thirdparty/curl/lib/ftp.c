...

{
  ...
  for(; fcmd != FTPPORT_DONE; fcmd = (ftpport)(fcmd + 1)) {

    if(!conn->bits.ftp_use_eprt && (FTPPORT_EPRT == fcmd))
      /* if disabled, goto next */
      continue;

    if((FTPPORT_PORT == fcmd) && sa->sa_family != AF_INET)
      /* PORT is IPv4 only */
      continue;
...
static CURLcode ftp_state_prepare_transfer(struct connectdata *conn)
{
  ...
  if(ftp->transfer != FTPTRANSFER_BODY) {
    ...
    result = ftp_state_use_port(conn, FTPPORT_EPRT);
  }
  else {
