...
psd_blend_mode psd_stream_get_object_blend_mode(psd_context * context)
{
	...
	else
	{
		...
		keychar[length] = 0;
		if(strcmp((const char*)keychar, "linearBurn") == 0)
			blend_mode = psd_blend_mode_linear_burn;
		else if(strcmp((const char*)keychar, "linearDodge") == 0)
			blend_mode = psd_blend_mode_linear_dodge;
		else if(strcmp((const char*)keychar, "vividLight") == 0)
			blend_mode = psd_blend_mode_vivid_light;
		else if(strcmp((const char*)keychar, "linearLight") == 0)
			blend_mode = psd_blend_mode_linear_light;
		else if(strcmp((const char*)keychar, "pinLight") == 0)
			blend_mode = psd_blend_mode_pin_light;
		else if(strcmp((const char*)keychar, "hardMix") == 0)
