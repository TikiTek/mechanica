...
static size_t ns_out(struct ns_connection *nc, const void *buf, size_t len) {
  if (nc->flags & NSF_UDP) {
    long n = sendto(nc->sock, (const char *) buf, (int)len, 0, &nc->sa.sa,
                    sizeof(nc->sa.sin));
...
#endif
  { n = (int) send(conn->sock, io->buf, (int)io->len, 0); }
...
void ns_broadcast(struct ns_mgr *mgr, ns_callback_t cb,void *data, size_t len) {
  ...
  if (mgr->ctl[0] != INVALID_SOCKET && data != NULL &&
    ...
    send(mgr->ctl[0], (char *) &ctl_msg, (int)(
...
struct connection {
  ...
  size_t request_len;  // Request length, including last \r\n after last header
};
...
static int wait_until_ready(sock_t sock, int for_read) {
  ...
  FD_ZERO(&set);
  FD_SET(sock, &set);
  select((int)sock + 1, for_read ? &set : 0, for_read ? 0 : &set, 0, 0);
  return 1;
}
...
static void abs_path(const char *utf8_path, char *abs_path, size_t len) {
  ...
  WideCharToMultiByte(CP_UTF8, 0, buf2, (int)wcslen(buf2) + 1, abs_path, (int)len, 0, 0);
}
...
static void on_cgi_data(struct ns_connection *nc) {
  ...
  if (conn->ns_conn->flags & NSF_BUFFER_BUT_DONT_SEND) {
    ...
    size_t len = get_request_len(io->buf + s_len, io->len - s_len);
...
static size_t parse_http_message(char *buf, size_t len, struct mg_connection *ri) {
  ...
  ri->content_len = ri->num_headers = ri->status_code = ri->is_websocket = 0;
...
int mg_match_prefix(const char *pattern, size_t pattern_len, const char *str) {
  const char *or_str;
  int len, res, i = 0, j = 0;
...
#endif
  if (conn->num_bytes_recv >= (int64_t)(conn->cl + conn->request_len) &&
...
static void close_local_endpoint(struct connection *conn) {
  ...
  c->content_len = c->num_headers = c->status_code = c->is_websocket = 0;
