...
namespace tiki
{
	...
	TransitionState BasicTestState::processTransitionStep( uintreg currentStep, bool isCreating, bool isInital )
	{
		switch ( currentStep )
		{
	...
	void BasicTestState::render( GraphicsContext& graphicsContext )
	{
		...
		m_renderer.drawRectangle( AxisAlignedRectangle::create( 50.0f, 50.0f, 100.0f, 100.0f ), aColors[ (colorIndex + 1u) % TIKI_COUNT( aColors ) ] );
		m_renderer.drawTexturedRectangle( m_pTestTexture->getTextureData(), AxisAlignedRectangle::create( 200.0f, 50.0f, 100.0f, 100.0f ), TIKI_COLOR_WHITE );
		m_renderer.drawTexturedRectangle( m_pTestFont->getTextureData(), AxisAlignedRectangle::create( 350.0f, 50.0f, 256.0f, 256.0f ), TIKI_COLOR_WHITE );
		m_renderer.drawTexturedRectangle( m_pTestFont->getTextureData(), AxisAlignedRectangle::create( 350.0f, 350.0f, 20, 20), AxisAlignedRectangle::create( 140.0f, 20.0f, 20.0f, 20.0f ), TIKI_COLOR_WHITE );
		m_renderer.drawTexturedRectangle( m_pTestFont->getTextureData(), AxisAlignedRectangle::create( 350.0f, 400.0f, 8, 9 ), AxisAlignedRectangle::create( 147.0f, 21.0f, 8.0f, 9.0f ), TIKI_COLOR_WHITE );
